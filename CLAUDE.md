...
1. Copy `.env.example` to `.env` in the project root and fill in real values, or set them as
   real process/web-server environment variables instead - both work (`app/config.php`'s
   `loadEnvFile()` loads `.env` on every request if present, and a value there always wins over
   one already set in the environment, so never deploy a `.env` file to production; it's
   `.gitignore`d for exactly that reason). Required: `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`,
...
4. Entry points: `/` (public site, served from `site/index.php` - language is chosen dynamically
   ...
   `/backoffice/` (role-scoped SaaS admin), `/portal/` (customer self-service).
