...
/** Human-readable file size ("5 MB" instead of a raw byte count) for upload-limit hints. */
function formatFileSize($bytes)
{
    $bytes = (float) $bytes;
    $units = [[1048576, 'MB'], [1024, 'KB']];
    foreach ($units as [$divisor, $unit]) {
        if ($bytes >= $divisor) {
            $value = $bytes / $divisor;
            $formatted = fmod($value, 1) === 0.0 ? (string) (int) $value : number_format($value, 1);
