...
$damages = $canIncidents ? dbFetchAll('SELECT id,damage_type,severity,status,estimated_cost,created_at FROM vehicle_damages WHERE vehicle_id=:vehicle ORDER BY created_at DESC LIMIT 10',['vehicle'=>$vehicleId]) : [];
...
<section class="detail-header vehicle-hero">
  <div class="vehicle-hero-main">
    ...
    <div><h2><?=e($vehicle['brand'].' '.$vehicle['model'])?></h2><p><?=isolatedValue($vehicle['registration_number'],'registration-value')?> · <?=e($vehicle['category_name'])?></p><div class="vehicle-hero-badges"><?=statusBadge($vehicle['status'])?><span><?=e(number_format((int)$vehicle['current_mileage']))?> <?=e(t('unit.km'))?></span><span><?=money($vehicle['base_daily_price'])?> / <?=e(t('unit.day'))?></span></div></div>
  </div>
