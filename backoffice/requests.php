...
<?php foreach($requests as $request):?><tr><td><?=formattedDateTime($request['created_at'])?></td><td><?=e($request['first_name'].' '.$request['last_name'])?></td><td><?=isolatedValue($request['reference'],'reference-value')?></td><td><?=e(translatedStatus($request['request_type']))?></td><td class="wrap-cell"><?=e($request['message'])?></td><td><?=statusBadge($request['status'])?></td><td><?php if($request['status']==='pending'):?><?=actionMenu([['label'=>'action.resolve','drawer'=>'#resolve-request-'.$request['id'],'drawer_title'=>'action.resolve']])?><template id="resolve-request-<?=e($request['id'])?>"><form method="post"><?=csrfField()?><input type="hidden" name="id" value="<?=e($request['id'])?>"><label><?=e(t('field.decision'))?><select name="decision"><?php foreach(['approved','rejected'] as $decision):?><option value="<?=e($decision)?>"><?=e(translatedStatus($decision))?></option><?php endforeach;?></select></label><label><?=e(t('field.notes'))?><input name="notes"></label><button class="btn primary"><?=e(t('action.resolve'))?></button></form></template><?php else:?><?=e($request['resolution_notes'])?><?php endif;?></td></tr><?php endforeach;?></tbody></table><?php if(!$requests) echo emptyState('empty.no_requests'); ?></div>
