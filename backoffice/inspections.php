...
if(requestMethod()==='POST'){
    requireCsrfPost();
    try {
        $action=(string)($_POST['action']??'');if($action==='rental_checkout'&&!can('rental.checkout')){http_response_code(403);exit(t('validation.not_authorized'));}
    ...
    } catch(Throwable $e) { if($e instanceof AuthorizationException)flash('danger',t('validation.not_authorized'));elseif($e instanceof InvalidArgumentException||$e instanceof DomainException)flash('danger',$e->getMessage());else{error_log('Phase 5B.3 checkout/controller failure: '.get_class($e));flash('danger',t('validation.checkout_failed'));} }
