...
function inspectionPhotoVerifyActiveBundle(int $inspectionId,int $agencyId): array {
    ...
    $slots=array_column($rows,'photo_slot');$required=inspectionPhotoSlots();sort($slots,SORT_STRING);sort($required,SORT_STRING);if(count($rows)!==count($required)||$slots!==$required)throw new DomainException(t('validation.checkout_photo_bundle'));
    foreach($rows as$row){$path=inspectionPhotoPath((string)$row['storage_path']);if(!$path||!is_file($path)||is_link($path)||(int)filesize($path)!==(int)$row['file_size']||!hash_equals((string)$row['sha256'],hash_file('sha256',$path)))throw new DomainException(t('validation.checkout_photo_bundle'));$mime=(new finfo(FILEINFO_MIME_TYPE))->file($path);if(!in_array($mime,['image/jpeg','image/png','image/webp'],true)||!hash_equals((string)$row['mime_type'],$mime))throw new DomainException(t('validation.checkout_photo_bundle'));}
    return $rows;
}
